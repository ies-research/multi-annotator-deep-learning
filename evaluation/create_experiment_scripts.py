...
data_set_name_list = ["letter", "fmnist", "cifar10", "cifar100", "svhn", "label-me"]
data_type_dict = {
...
lr_dict = {
    "letter": [0.01, 0.005, 0.001],
    "fmnist": [0.01, 0.005, 0.001],
    "cifar10": [0.01, 0.005, 0.001],
    "cifar100": [0.1, 0.01],
    "svhn": [0.01, 0.005, 0.001],
    "label-me": [0.01, 0.005, 0.001],
    "music": [0.01, 0.005, 0.001],
}
weight_decay_dict = {
    "letter": [0.0001, 0.001, 0],
    "fmnist": [0.0001, 0.001, 0],
    "cifar10": [0.0001, 0.001, 0],
    "cifar100": [0.0005, 0],
    "svhn": [0.0001, 0.001, 0],
    "label-me": [0.0001, 0.001, 0],
    "music": [0.0001, 0.001, 0],
}


def get_lr_schedular_params(max_epochs, data_set_name):
    return f"lr_scheduler=CosineAnnealing lr_scheduler_dict.T_max={max_epochs}"


def get_optimizer_params(data_set_name, lr, weight_decay):
    s = f"optimizer={optimizer_dict[data_set_name]} optimizer_dict.lr={lr} optimizer_dict.weight_decay={weight_decay}"
    if data_set_name == "cifar100":
        s += f" optimizer_dict.momentum={0.9}"
    return s
...
ovat_dict = {
    ...
    "ap_use_res": {"none": [False], "learned": [True, False]},
    "ap_use_op": {"none": [False], "learned": [True, False]},
}
...
for data_set_name, data_type_list in data_type_dict.items():
    for data_type in data_type_list:
        ...
        for mlr in missing_label_dict[data_set_name][data_type]:
            for lr in lr_dict[data_set_name]:
                for weight_decay in weight_decay_dict[data_set_name]:
                                    ...
                                    for param, value_list in ovat_dict.items():
                                        ...
                                        for value in value_list:
                                            commands.append(
                                                ...
                                                f"{get_optimizer_params(data_set_name, lr, weight_decay)} "
                                                f"{get_lr_schedular_params(max_epochs, data_set_name)} "
                                                ...
                                                f"model_dict.ap_use_residual={ovat_dict['ap_use_res'][embed_x][0]} "
                                                f"model_dict.ap_use_outer_product"
                                                f"={ovat_dict['ap_use_op'][embed_x][0]}"
                                            )
                                            if (
