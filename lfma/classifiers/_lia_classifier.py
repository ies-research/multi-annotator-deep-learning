...
from ..modules import MultiAnnotatorDataSet, EMMultiAnnotatorDataSet, LIAModule
from ..utils import check_annotator_features


class LIAClassifier(LightningClassifier):
    """LIAClassifier
    ...
    def fit(
        ...
        A : array-like of shape (n_samples, *), optional (default=None)
            The matrix `A` is the annotator feature matrix. The exact form the on the `module_class`.
        ...
        -------
        self: LIAClassifier,
            `LIAClassifier` object fitted on the training data.
        """
        ...
        train_data_set = EMMultiAnnotatorDataSet(X=X, y=y, A=A, transform=transform)
