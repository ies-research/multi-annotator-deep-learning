...
from skactiveml.utils import is_labeled, check_type
...
from ..modules import AggregateModule, MultiAnnotatorDataSet
from ..utils import check_annotator_features


class AggregateClassifier(LightningClassifier):
    """AggregateClassifier
    ...
    def fit(
        ...
        y_agg : array-like of shape (n_samples,)
            Aggregated class label for each sample.
        ...
        Returns
        -------
        self: AggregateClassifier,
            `AggregateClassifier` object fitted on the training data.
        """
        # Check input parameters.
        X, y, y_agg, A, X_val, y_val, data_loader_dict, val_data_loader_dict = self._validate_data(
            X=X,
            y=y,
            A=A,
            y_agg=y_agg,
    ...
    def predict_annotator_perf(
        ...
        return_cond : bool, optional (default=False)
            If `return_cond=True`, the confusion matrix of annotator describes conditional probabilities, otherwise
            joint probabilities.
        data_loader_dict : dict or None (default=None)
            Dictionary passed to the DataLoader object for the training samples `X`. If `None`, an empty dictionary is
            passed.
    ...
    def compute_annotator_embeddings(self, A=None):
        A = self.A_ if A is None else torch.tensor(A).float()
        return self.module_.ap_embed_a(A).numpy()

    @torch.no_grad()
    def _validate_data(
        self, X, y, y_agg, A=None, X_val=None, y_val=None, data_loader_dict=None, val_data_loader_dict=None
    ):
        X, y, X_val, y_val, data_loader_dict, val_data_loader_dict = super()._validate_data(
            ...
            data_loader_dict=data_loader_dict,
            val_data_loader_dict=val_data_loader_dict,
            remove_unlabeled_samples=False,
        )
        y_agg = self._le.transform(y_agg)
        is_lbld = is_labeled(y_agg, missing_label=-1)
        X = torch.tensor(X[is_lbld]).float()
        y = torch.tensor(y[is_lbld]).long()
        y_agg = torch.tensor(y_agg[is_lbld]).long()
        ...
        return X, y, y_agg, self.A_, X_val, y_val, data_loader_dict, val_data_loader_dict
